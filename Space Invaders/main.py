...
class Player(Ship):
	def __init__(self, x, y, health=100):
		...
		self.ship_img = PLAYER_YELLOW_SHIP
...
def main():
	...
	while run:
		...
		if keys[pygame.K_a] and player.x - player_vel > 0:
			player.x -= player_vel 
		if keys[pygame.K_s] and player.y - player_vel + player.get_width() +20 < HEIGHT:
			player.y += player_vel
		if keys[pygame.K_d] and player.x - player_vel + player.get_height() < WIDTH: 
			player.x += player_vel
...
def main_menu():
	...
	while run:
		WIN.blit(BG, (0,0))
		title_label = title_font.render("Press WASD or SPACE to begin", 1, (255,255,255))
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_w or pygame.K_a or pygame.K_s or pygame.K_d or pygame.K_SPACEE:
					main()
	pygame.quit()
